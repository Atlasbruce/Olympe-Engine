...
void BTNodePalette::Render(bool* isOpen) {
     // Render palette content directly (don't wrap in BeginChild - already in a tab)
     // Renders search filter and categorized node buttons for drag-drop

     // Search filter
     ImGui::InputText("Search##BTSearch", m_searchFilter, sizeof(m_searchFilter));
     ImGui::Separator();

     // Render by category using CollapsingHeaders
