    ...
    GraphDocument* doc)
{
    ...
    const std::vector<LinkData>& allLinks = doc->GetLinks();
    for (size_t i = 0; i < allLinks.size(); ++i) {
        const LinkData& link = allLinks[i];
...
std::vector<NodeId> AIEditorClipboard::Paste(
	GraphDocument* doc,
	Vector pasteOffset)
{
	...
	{        
		...
		NodeId newId = doc->CreateNode(m_nodes[i].type, pos);

		// Set name and parameters
		const NodeData* nodeData = doc->GetNode(newId);
		if (nodeData != nullptr) {
			NodeData updatedData = *nodeData;
