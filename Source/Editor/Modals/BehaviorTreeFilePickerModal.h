/**
 * @file BehaviorTreeFilePickerModal.h
 * @brief ImGui modal for selecting BehaviorTree files (Phase C).
 * @author Olympe Engine
 * @date 2026-03-15
 *
 * Provides a file browser dialog for users to select .bt.json behavior tree files
 * to link with AIBehavior_data components in EntityPrefab editor.
 * Pattern adapted from SubGraphFilePickerModal.
 */
 ...
 * @brief ImGui modal dialog for selecting BehaviorTree .bt.json files.
 *
 * Usage:
...
private:
    ...
    std::vector<std::string> m_behaviorTreeFiles;  ///< .bt.json files in current directory
    ...
    /**
     * @brief Scans current directory for .bt.json behavior tree files.
